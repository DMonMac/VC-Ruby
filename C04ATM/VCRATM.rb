  ...
  @clients.each do |wd|
    if @pin.strip.to_i == wd[:pin]
      ...
      if @wthdrwl.to_f.round(2) == 0
        puts "\n"*2+"You withdrew nothing.".center(92)
        gets
        newtrans
        ...
        puts "\n"*2+"Prev. Bal = #{wd[:bal].to_f.round(2)} Php".center(92)+"\n"
        puts "Withdrawn = #{@wthdrwl.to_f.round(2)} Php".center(92)+"\n"
        puts "Curr. Bal = #{wd[:bal].to_f.round(2)-@wthdrwl.to_f.round(2)} Php".center(92)+"\n"
  ...
  @clients.each do |dep|
    if @pin.strip.to_i == dep[:pin]
      ...
      if @dpst.to_f.round(2) == 0
        puts "\n"*2+"You deposited nothing.".center(92)
        gets
        newtrans
      else
        puts "\n"*2+"Prev. Bal = #{dep[:bal].to_f.round(2)} Php".center(92)+"\n"
        puts "Deposited = #{dep[:bal].to_f.round(2)} Php".center(92)+"\n"
        puts "Curr. Bal = #{dep[:bal].to_f.round(2)+@dpst.to_f.round(2)} Php".center(92)+"\n"
...
def chngpin
  ...
  @clients.each do |oldpinchk|
    if @oldpin.strip.to_i == oldpinchk[:pin]
      ...
      puts "\n"*3+'Your pin has been changed. Machine will now return to opening screen.'.center(92)+"\n"*3
      gets
      xPrg('end')
    end
  end
  puts "\n"*3+'Invalid PIN.'.center(92)+"\n"*3
  gets
  chngpin
end
...
def fndtrns
  ...
  puts '(Type "cancel" to cancel this transaction.)'.center(92)+"\n"*2
  puts '(Type "End" to abort transaction.)'.center(92)+"\n"*5
  puts 'Enter Account Number to transfer funds to:'.center(92)
  print (' ').center(40)
  @ftactnum = gets
  ...
  puts "\n"*3+'Invalid Account Number.'.center(92)+"\n"*3
  gets
  fndtrns
